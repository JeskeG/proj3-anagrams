...
def checkword():
    """
    Checks to see if input string uses the letters in the scramble
    and creates a word from the word list.
    """
    ...
    # Does text use the jumbled letters?
    in_jumble = LetterBag(jumble).contains(text)
    #Is text a word in the matches?
    ...
    if matched and in_jumble and not (text in matches):
        ...
        rslt = {"matches": " ".join(matches)}  #, "success": success}
        return flask.jsonify(result=rslt)

    elif text in matches:
        app.logger.debug("Already found this word")
    elif not matched:
        app.logger.debug("Word not in the matches")
    elif not in_jumble:
        app.logger.debug("Letters typed not in the jumble")
    else:
